...
// ------------ Delete Function to delete a perticular Node --------------->
node* del(node *start)
    {
        ...
        scanf("%d",&num);
        i=search(start,num); // calling the search() function
        if(i==NULL)
        {
            printf("\n\n\t This number does not exits in the list");
            getch();
    return(start);
        }
        ...
        do
        {
            if(temp==start && temp->link==start && temp->data==num)
                {
                    free(start);
                    start=NULL;
                            break;
